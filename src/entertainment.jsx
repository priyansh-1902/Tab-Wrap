...
function Entertainment() {
  const {
    ...
    categoryNames: ['Entertainment', 'Entertainment / Streaming', 'Movies', 'TV'],
    label: 'SPENT ON ENTERTAINMENT',
    focusColor: '#f472b6',
    streakKey: '',
    streakTitle: 'TOP ENTERTAINMENT STREAK',
    streakBarColor: 'bg-pink-400',
    streakHoverColor: 'hover:bg-pink-300',
    streakGlowColor: 'rgba(244, 114, 182, 0.8)',
    summaryType: 'entertainment',
    quipDefault: 'Enjoy the show!'
  });
  ...
  return (
    <div className="min-h-screen p-4 sm:p-8 flex items-center justify-center bg-gray-900 font-sans">
      ...
      <div className="absolute inset-0 overflow-hidden -z-10">
        <div className="absolute top-10 left-1/4 w-96 h-96 bg-pink-400 rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-blob"></div>
      ...
      <div className="w-full max-w-xl p-6 md:p-8 rounded-3xl backdrop-filter backdrop-blur-xl bg-white/5 border border-white/10 shadow-2xl relative space-y-12">
        ...
        <h1 className="text-3xl sm:text-4xl font-extrabold text-center text-white uppercase tracking-widest drop-shadow-lg">
          ENTERTAINMENT CATEGORY SUMMARY
        </h1>
        ...
        <div className="flex flex-col items-center justify-center p-6 bg-white/10 rounded-2xl border border-white/10 shadow-lg w-full max-w-lg mx-auto my-4">
          <p className="text-base sm:text-lg font-semibold text-white text-center">
            {isStreaming ? <span>{quippySummary}<span className="animate-pulse">|</span></span> : (quippySummary || 'Enjoy the show!')}
          </p>
        </div>
        ...
        <StreakChart
          ...
          title={'TOP ENTERTAINMENT STREAK'}
        />
...
createRoot(root).render(<Entertainment />);
